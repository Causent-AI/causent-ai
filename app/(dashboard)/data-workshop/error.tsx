...
export default function DataWorkshopError({
  error,
  unstable_retry,
}: {
  ...
  useEffect(() => {
    console.error("[data-workshop] route render failed", error);
  }, [error]);

  return (
    <section
      className="mx-auto my-10 w-[calc(100%_-_2rem)] max-w-xl rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm"
      role="alert"
    >
      <h1 className="text-[24px] font-semibold text-[var(--text)]">
        Data Workshop could not be loaded
      </h1>
      <p className="mt-2 text-[13px] leading-6 text-[var(--text-muted)]">
        No metrics or observations were changed. Try loading the workspace catalog again.
      </p>
      {error.digest ? (
