...
export function getMetricDelta(metric: Metric, lookback = 30): MetricDelta {
  ...
  const good = metric.beneficialDirection === "unknown" || metric.beneficialDirection === "neutral" ? null : direction === "neutral" ? true : (direction === "up") === metric.higherIsBetter;

  let magnitude: string;
  if (metric.format === "currency") magnitude = formatCurrencyDelta(abs);
  else if (metric.format === "percent") magnitude = metric.percentScale === "unknown" ? `${abs.toPrecision(3)} stored units (scale unconfirmed)` : formatPpDelta(metric.percentScale === "ratio" ? abs * 100 : abs);
  ...
  return {
    latest,
    latestLabel: formatMetricValue(latest, metric.format, metric.percentScale),
    direction,
    good,
