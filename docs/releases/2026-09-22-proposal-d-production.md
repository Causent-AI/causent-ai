...
Gateway diagnostics confirm the native structured-output grammar exceeds the provider limit, causing two 400 responses followed by malformed Bedrock output. Stronger prompt instructions alone did not resolve it. The follow-up selects Anthropic `structuredOutputMode: jsonTool`, keeping Sonnet, Gateway routing, validation, admission and provenance unchanged. Three approved requests have run ($0.0649 total); one remains. Both owner and fresh-account fallback drafts retain editable input. Owner activation and all six handoff previews pass.
