...
#37 remains live as `9dcc20b` on `app.causent.ai` (`dpl_8erw9kJrgjcWyo5CqpFy5cCYxDoD`). [PR #38](https://github.com/Causent-AI/causent-ai/pull/38) is receiving a design-fidelity correction: the original port was incomplete. The corrected metric library, documents/onboarding, compact Actions/Impact, branded shell and full dark graph pass local checks (735 app/database, 1,338 engine/RLS/bridge, 27 prototype/load tests, types, lint and build). Backend contracts and disabled GA4 are preserved. A replacement candidate must pass hosted acceptance. The owner approved the bounded test run. Five of six approved AI requests have run: live rewriting passes, but full-report generation remains blocked on provider output validation. A simplified native schema is under verification. The isolated test workspaces are temporarily restored; production remains unchanged. See the [engineering review](reviews/2026-09-22/PRODUCTION_DESIGN_REVIEW.md) and [release manifest](releases/2026-09-22-proposal-d-production.md). Older entries are historical.
