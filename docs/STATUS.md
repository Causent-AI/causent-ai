...
#37 remains live as `9dcc20b` on `app.causent.ai` (`dpl_8erw9kJrgjcWyo5CqpFy5cCYxDoD`). [PR #38](https://github.com/Causent-AI/causent-ai/pull/38) restores the approved Proposal D layouts. Both-account editing, charts, two-metric/three-action activation and all twelve handoff previews pass. The owner authorized continued tests/fixes. Compact-schema live generation now passes, including three actions and saved reload. A rewrite-label correction and reduced reasoning effort pass 743 local app/database tests; their live acceptance remains pending. Only the fresh synthetic workspace is temporarily restored; original data/audit digests are unchanged, and GA4 stays disabled. No merge or promotion occurred. See the [engineering review](reviews/2026-09-22/PRODUCTION_DESIGN_REVIEW.md) and [release manifest](releases/2026-09-22-proposal-d-production.md). Older entries are historical.
