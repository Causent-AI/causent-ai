(() => {
  ...
  function usesSampleImpact(){return state.project.sample&&(state.report.estimate?.metricId||state.report.coreMetricIds[0]||metrics[0]?.id)==='activation';}
  function renderImpact() {
    ...
    const sampleReadout=state.project.sample&&primary?.id==='activation';
  ...
  function switchTab(tab) {
    if(!['data','reports','actions','impact','projects','onboarding'].includes(tab))return;
    state.tab=tab;closePopovers();
    $('preview-note').textContent='Proposal C · '+(tab==='onboarding'?'Local preview':state.project.sample?'Sample data':'Local project')+' · Edits stay until reload';
  ...
  function activateProject(id) {
    ...
    $('profile-context').textContent='Proposal C · '+(project.sample?'Sample workspace':'Local workspace');
    renderProjectName();
    $('preview-note').textContent='Proposal C · '+(project.sample?'Sample data':'Local project')+' · Edits stay until reload';
  ...
  $('keep-preview').addEventListener('click',()=>{
    ...
    if(preview.draft&&state.report.title==='Untitled report')state.report.title=state.project.sample?'A simpler first session.':state.project.name+': Overview';
