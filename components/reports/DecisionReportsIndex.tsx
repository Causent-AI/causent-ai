...
function lifecycleLabel(report: DashboardDecisionReport): string {
  if (report.isCurrent) return "Current";
  if (report.status === "active") return "Previous";
  ...
  return (
    <details className="relative">
      ...
      <form action={action} className="absolute right-0 z-20 mt-2 w-80 rounded-xl border border-[var(--border)] bg-white p-4 shadow-xl">
        ...
        <label className="text-[12px] font-semibold" htmlFor={`iteration-reason-${reportId}`}>What changed?</label>
        <p className="mt-1 text-[11px] leading-4 text-[var(--text-muted)]">The current version stays live until this one is activated. Images must be reattached.</p>
        <textarea id={`iteration-reason-${reportId}`} name="reason" required minLength={1} maxLength={500} className="mt-3 min-h-20 w-full rounded-lg border border-[var(--border)] p-2 text-[12px]" />
        {state.status === "error" ? <p role="alert" className="mt-2 text-[11px] text-red-700">{state.error}</p> : null}
        <button disabled={pending} className="mt-3 w-full rounded-lg bg-[var(--brand-blue)] px-3 py-2 text-[12px] font-semibold text-white disabled:opacity-50">{pending ? "Creating…" : "Create draft"}</button>
      </form>
    </details>
  );
}
            ...
            >
              <div className="flex items-center justify-between gap-2 px-1.5 pb-1.5 pt-0.5">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-[var(--text-subtle)]">
                  {series.hasCurrent ? "Current" : series.hasActive ? "Previous" : "Draft"}
                </p>
                <span className="text-[10px] text-[var(--text-subtle)]">
                  {series.reports.length} {series.reports.length === 1 ? "version" : "versions"}
                </span>
              </div>
                ...
                {series.reports.map((report) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="min-w-0 flex-1 truncate text-[14px] font-medium">Version {report.iterationNumber}: {report.title}</span>
